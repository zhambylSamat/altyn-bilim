...
if(isset($_SESSION['student_num']) && isset($_GET[md5(md5('dataNum'))])){
	try {
		$stmt_test_num = $conn->prepare("SELECT st.topic_num stTopicNum, t.test_num tNum FROM test t, subtopic st WHERE t.subtopic_num = :subtopic_num AND st.subtopic_num = t.subtopic_num");
