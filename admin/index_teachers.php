...
<table class="table table-striped table-bordered">
	...
	?>
	<tr class='head'>
		<td style='width: 5%;'><center><h4><?php echo $teacher_number;?></h4></center></td>
		<td style='width: 75%'>
			...
			<div class='user_info'>
				<table class='' style='width:100%; background-color:rgba(0,0,0,0); margin:0; padding:0; border:none;'>
					<tr>
						<td style='width: 40%'><h4 class='text-success'><?php echo $readrow['surname']?>&nbsp;<?php echo $readrow['name']?></h4></td>
		...
		<td style='width: 20%;'>
			<form onsubmit="return confirm('Вы точно хотите удалить учителя? Все данные об учителе будут удалены.')" action='admin_controller.php' method='post'>
				<center>
					...
					<a class='btn btn-default btn-sm edit_user' title='Өзгерту'>
					...
					<a href="schedule.php?data_num=<?php echo $readrow['teacher_num'];?>" target="_blank" title='Сабақ кестесі' class='btn btn-sm btn-default'><span style='font-size: 20px; cursor: pointer;' class='glyphicon glyphicon-calendar text-info'></span></a>
				</center>
			</form>
		</td>
	</tr>
